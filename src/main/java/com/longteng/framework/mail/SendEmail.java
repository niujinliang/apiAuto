...
import com.longteng.framework.report.template.MailReport;
...
public class SendEmail {
    ...
    private static Config config = RunTest.getConfig();
...
//        properties.setProperty("mail.debug", "true");
        ...
        // 设置邮件的发件人
        message.setFrom(new InternetAddress(config.get("fromMail")));

        String sendTo = config.get("sendTo");
        String sendToCC = config.get("sendToCC");

        // 设置邮件的收件人 cc表示抄送 bcc 表示暗送
        if (null != sendTo) {
            InternetAddress[] senderList;
            senderList = new InternetAddress().parse(sendTo);
            message.setRecipients(Message.RecipientType.TO, senderList);
        } else {
            return;
        }
        if (null != sendToCC) {
            InternetAddress[] iaCCList = new InternetAddress().parse(sendToCC);
        ...
        if (subject.equalsIgnoreCase("")) {
            ...
            message.setSubject("自动化测试报告","UTF-8");
        ...
        text.setContent(MailReport.reportHtml() + "<img src='cid:b'>",
